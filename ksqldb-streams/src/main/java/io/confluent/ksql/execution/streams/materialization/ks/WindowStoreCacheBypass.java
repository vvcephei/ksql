...
public class WindowStoreCacheBypass {
  ...
  private static final Field SERDES_FIELD;

  static {
    try {
      CACHING_WINDOW_STORE_CLASS = Class.forName(CACHING_WINDOW_STORE_CLASS_NAME);
    } catch (final ClassNotFoundException e) {
      throw new RuntimeException("Can't find " + CACHING_WINDOW_STORE_CLASS_NAME, e);
    }
    try {
  ...
  private static WindowStoreIterator<ValueAndTimestamp<GenericRow>> fetchUncached(
    ...
    if (windowStore instanceof MeteredWindowStore) {
      ...
      while (wrapped instanceof WrappedStateStore) {
        wrapped = (WindowStore<Bytes, byte[]>) ((WrappedStateStore<?, ?, ?>) wrapped).wrapped();
      }
